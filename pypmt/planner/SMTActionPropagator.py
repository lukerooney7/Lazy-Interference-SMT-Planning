...
class SMTSearchActionPropagator(Search):
    ...
    def search(self):
        ...
        for horizon in self.scheduler:
            ...
            log(f'Step {horizon + 1}/{(self.scheduler[-1] + 1)} encoding: {encoding_time:.2f}s, solving: {solving_time:.2f}s',
                2)
            if res == z3.sat:
    ...
    def dump_smtlib_to_file(self, t, path):
        ...
        encoding_time = end_time - start_time
        self.solver.add(
            g)  # type: ignore # we assert the goal happens in the last step (which would normally be an assumption)
