                ...
                if source in self.current[step]:
                    ...
                    while len(to_explore) > 0:
                        ...
                        if source == node or node in self.ancestors[source]:
                            cycle = self.ancestors[dest] & self.descendants[dest]
                            cycle = {self.encoder.get_action_var(a, step) for a in cycle}
                            cycle.add(self.encoder.get_action_var(source, step))
                            cycle.add(self.encoder.get_action_var(dest, step))
                            self.conflict(deps=list(cycle), eqs=[])
                            break
