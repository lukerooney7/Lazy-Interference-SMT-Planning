        ...
        if value:
            ...
            for source, dest in self.graph.edges(action_name):
                if dest in self.current[step]:
                    ...
                    for s in range(step):
                        ancestors = {self.encoder.get_action_var(a, s) for a in self.ancestors[source]}
                        ancestors.add(self.encoder.get_action_var(dest, s))
                        ancestors.add(self.encoder.get_action_var(source, s))
                        self.propagate(e=z3.Not(z3.And(*ancestors)), ids=[], eqs=[])
