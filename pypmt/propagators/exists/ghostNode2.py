        ...
        if value:
            ...
            for source, dest in self.graph.edges(action_name):
                if dest in self.current[step]:
                    ...
                    s = {neighbor for node in self.graph.neighbors(dest) for neighbor in self.graph.neighbors(node)}
                    if s & self.ancestors[source] or source in s:
                        self.propagate(e=z3.Not(
